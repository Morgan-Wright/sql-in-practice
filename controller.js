...
module.exports = {
    ...
    getAllClients: (req, res) => {
        sequelize.query(`SELECT * FROM cc_users AS u JOIN cc_clients AS c ON c.user_id = u.user_id;`)
        .then(dbRes => res.status(200).send(dbRes[0]))
        .catch(err => console.log(err))
    },

    getPendingAppt: (req, res) => {
    ...
    getPastAppt: (req, res) => {
        ...
        WHEREa.approved = true AND a.completed = false
        ORDER BY a.date DESC;`)
        .then(dbRes => res.status(200).send(dbRes[0]))
        .catch(err => console.log(err))
    },

    completeAppt: (req, res) => {
        sequelize.query(`UPDATE cc_appointments SET approved = true
        WHERE appt_id = ${apptId};`)
        .then(dbRes => res.status(200).send(dbRes[0]))
        .catch(err => console.log(err))
